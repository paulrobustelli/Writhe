...
trajectory = "./apo_asyn/biorxiv2021-10398286-asyn_cterm_apo_no-water/asyn_no_ions.dcd"
pdb = "./apo_asyn/biorxiv2021-10398286-asyn_cterm_apo_no-water/asyn_no_ions.pdb"
outdir='./apo_asyn/outdir/'
...
# need this workflow because atoms are misplaced 
...
topology = md.Topology.from_dataframe(top,bonds)
...
def plot_avg_matrx(matrx, title="Writhe, Average", count=0): 
    ...
    plt.savefig(outdir+sys+"_avg_"+title[0]+str(count)+".png")
    plt.clf()
    return
...
def plot_project_time(PC1, PC2, time=50, title= "writhe PCA"):    
    ...
    plt.savefig(outdir+sys+"_project_time_"+title[0]+".png") # could change this later? 
    plt.clf()
    return
...
def plot_project_rg(PC1, PC2, title= "writhe PCA"): 
    ...
    plt.savefig(outdir+sys+"_project_rg_"+title[0]+".png")
    plt.clf() 
    return 
...
def plot_free_energy(PC1, PC2, col_map, centers=None, scat=False, title="Free Energy, Writhe", xmin=-3, xmax=3, ymin=-3, ymax=3):
    ...
    if scat: 
        ...
        plt.scatter(scat[0], scat[1], c="red", marker = "x", s=100)
    
    plt.axes(cb.ax)
    plt.clim(vmin=0.01, vmax=8.0)
    plt.yticks(size='16')
    plt.tight_layout()
    plt.savefig(outdir+sys+"free_energy_"+title[0]+".png")
    plt.close(fig)
    return
...
def main():   
    ...
    trj.superpose(trj, 0)
    c_model_input, contacts, contact_avg = make_contact_map(res_num=20) # could change to len(CAlabel)? 
    ...
    plot_project_time(WPC1, WPC2, time=200, title= 'Writhe PCA: asyn, time') # title matters because 1st letter is in output png
    plot_project_rg(WPC1, WPC2, title= "Writhe PCA: asyn, Rg")
    # wcenter = kmeans_cluster(WPC1, WPC2, clusters=3, title = "Writhe, Kmeans Clustering")
    plot_free_energy(WPC1, WPC2, n_cmap, title= "Writhe PCA: asyn, Free Energy,", xmin=-2, xmax=3, ymin=-2, ymax=3)
    
    CPC1, CPC2 = pca(c_model_input)
    plot_project_time(CPC1, CPC2, time=200, title= 'Contacts PCA: asyn, time')
    plot_project_rg(CPC1, CPC2, title= "Contacts PCA: asyn, Rg")
    # ccenter = kmeans_cluster(CPC1, CPC2, title = "Contacts, Kmeans Clustering")
    plot_free_energy(CPC1, CPC2, n_cmap, title="Contacts PCA: asyn, Free Energy", xmin=-2, xmax=3, ymin=-2, ymax=3)
