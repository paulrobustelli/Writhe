...
trajectory = "./drkN-Charmm36m.dcd"
pdb = "./drkN.pdb"
outdir='./outdir_Charmm36m/'
...
if not os.path.exists('outdir_Charmm36m'):
    os.makedirs('outdir_Charmm36m')
...
def wr_tot(CAlabel, CApos):
    ...
    """
    dim = int((len(CAlabel) - 1)*(len(CAlabel - 2)/2) + 1)
...
def main():   
    ...
    trj.superpose(trj, 0)
    c_model_input, contacts, contact_avg = make_contact_map(res_num=59) # could change to len(CAlabel)? 
    ...
    plot_project_time(WPC1, WPC2, time=30, title= 'Writhe PCA: drkN, time') # title matters because 1st letter is in output png
    plot_project_rg(WPC1, WPC2, title= "Writhe PCA: drkN, Rg")
    # wcenter = kmeans_cluster(WPC1, WPC2, clusters=3, title = "Writhe, Kmeans Clustering")
    plot_free_energy(WPC1, WPC2, n_cmap, title= "Writhe PCA: drkN, Free Energy", xmin=-3, xmax=3, ymin=-2, ymax=3)
    
    CPC1, CPC2 = pca(c_model_input)
    plot_project_time(CPC1, CPC2, time=30, title= 'Contacts PCA: drkN, time')
    plot_project_rg(CPC1, CPC2, title= "Contacts PCA: drkN, Rg")
    # ccenter = kmeans_cluster(CPC1, CPC2, title = "Contacts, Kmeans Clustering")
    plot_free_energy(CPC1, CPC2, n_cmap, title="Contacts PCA: drkN, Free Energy", xmin=-3, xmax=3, ymin=-2, ymax=5)
