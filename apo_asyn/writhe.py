...
def plot_avg_matrx(matrx, title="Writhe, Average", count=0): 
    ...
    plt.savefig(outdir+sys+"avg_"+title[0]+str(count)+".png")
    plt.clf()
    return
...
def plot_project_time(PC1, PC2, time=50, title= "writhe PCA"):    
    ...
    plt.savefig(outdir+sys+"project_time_"+title[0]+".png") # could change this later? 
    plt.clf()
    return
...
def plot_project_rg(PC1, PC2, title= "writhe PCA"): 
    ...
    plt.savefig(outdir+sys+"project_rg_"+title[0]+".png")
    plt.clf() 
    return 
...
def plot_free_energy(PC1, PC2, col_map, title="Free Energy, Writhe", xmin=-3, xmax=3, ymin=-3, ymax=3):
    """
...
def main():   
    ...
    plot_free_energy(WPC1, WPC2, n_cmap, title="Writhe PCA: asyn, Free Energy,")
    plot_project_rg(WPC1, WPC2, title= "Writhe PCA: asyn, Rg")
    
    CPC1, CPC2 = pca(c_model_input)
    plot_project_time(CPC1, CPC2, time=200, title= 'Contact Distances PCA: asyn, time')
    plot_free_energy(CPC1, CPC2, n_cmap, title="Contact Distances PCA: asyn, Free Energy")
    plot_project_rg(CPC1, CPC2, title= "Contact Distances PCA: asyn, Rg")
    
    
    # save avg writhe plot 
    plot_avg_matrx(writhe_matrx, title="Writhe, Average", count=0)
    plot_avg_matrx(contact_avg, title="Contact Distance, Average", count=0)
    # Use the following functions in a notebook, to toggle between different clusters 
